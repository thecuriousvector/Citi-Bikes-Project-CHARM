...
''' Variables '''
dataDir_train = "/user/rdv253/DS/Baseline/Train/*"
dockID = 368  #Dock of interest


data = sc.textFile(dataDir_train)
...
data_train = dock_data_per_day.map(lambda (date, hour, isPm, bikes): (formatDate(date,hour,isPm),bikes))
...
train_df.to_csv('train_368.csv', sep='\t')
